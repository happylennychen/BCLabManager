...
namespace O2Micro.BCLabManager.Database
{
    public class DBManager
    {
        ...
        private SQLiteDriver SqliteDriver = new SQLiteDriver();
        public bool DebugMode = false;       //为true时，每次启动都会在CobraDocument/Database/DebugDB创建新的临时DB
        private string DBName = "BCLabv0.1.db3";
        ...
        private System.Timers.Timer flush_timer = new System.Timers.Timer();
        private bool isTimerBooked = false;
        ...
        private void GetSessionIDFromSessionTable(string module_name, ref int session_id, string device_index = "", string session_establish_time = "")
        {
        ...
        private void GetSessionSize(int session_id, ref int session_size)
        {
            string sql = "select count(*) from " + DataTableName + " where session_id = " + session_id.ToString() + ";";
            DataTable dt = new DataTable();
            int row = -1;
            SqliteDriver.ExecuteSelect(sql, ref dt, ref row);
            session_size = Convert.ToInt32(dt.Rows[0]["count(*)"]);
        }

        #region API
        public void DBInit(string folder)
        {
            try
            {
                if (!isTimerBooked)
                ...
                if (!File.Exists(SqliteDriver.DB_Path + SqliteDriver.DB_Name))
                {
        ...
        public void ExtensionRegister(string project_name)
        {
                Project_Name = project_name;
        }
        public void NewSession(string module_name, ref int session_id, string device_index = "", string session_establish_time = "")
        {
            try
            {
                lock (DB_Lock)
        ...
        public void ExecuteQuery(string sql, ref DataTable dt, ref int row)
        {
            try
            {
                lock (DB_Lock)
        ...
        #region For Scan SFL
        public void ScanSFLUpdateSessionSize(int session_id, ulong session_row_number)
        {
            try
            {
                lock (DB_Lock)
        ...
        public void ScanSFLGetSessionsInfor(string module_name, ref List<List<string>> records)
        {
            try
            {
                lock (DB_Lock)
        ...
        public void ScanSFLDeleteOneSession(string module_name, string session_establish_time)
        {
            try
            {
                lock (DB_Lock)
        ...
        public void ScanSFLGetOneSession(string module_name, string session_establish_time, ref DataTable dt)
        {
            try
            {
                lock (DB_Lock)
